    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-soft flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="flex items-start gap-4">
          <img
            src={doctor.avatarUrl || getDoctorAvatarUrl(doctor.name)}
            alt={doctor.name}
            onError={(e) => {
              e.currentTarget.onerror = null;
              e.currentTarget.src = getDoctorAvatarUrl(doctor.name);
            }}
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-3xl object-cover border border-slate-200 shrink-0 shadow-sm bg-slate-100"
          />
          ...
          <div className="space-y-4">
            ...
            {doctor.visitHistory && doctor.visitHistory.length > 0 ? (
              doctor.visitHistory.map((vis) => (
                <div
                  key={vis.id}
                  className="bg-white rounded-3xl p-5 border border-slate-100 shadow-soft space-y-3"
                >
